...
export default async function AdminFlagsPage() {
  ...
  const snap = await adminDb.collectionGroup('messages').where('flagged', '==', true).orderBy('createdAt', 'desc').get()

  const flagged = snap.docs.map((doc) => {
    const data = doc.data()
    return {
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 flex-wrap mb-1">
                ...
                {msg.slotId && (
                  <>
                    ...
                    <Link href={`/schedule/${msg.slotId}`} className="text-xs text-primary hover:underline">
                      View Game
                    </Link>
                  </>
                )}
        ...
        {flagged.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">No flagged messages. All clear.</div>
        )}
      </div>
    </div>
  )
